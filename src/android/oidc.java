...
public class oidc extends CordovaPlugin {
  ...
  private void refreshNotification()
  {
    try
    {
      String config = getConfData("connectionconfig");
      ...
      intent.putExtra("notify", json.has("notification")?json.getBoolean("notification"):false);
  ...
  private void setConnectionConfig(JSONArray data) {
    ...
    try
    {
      ...
      values.put("config", data.getJSONObject(0).toString());
  ...
  private String getConfData(String type)
  {
    try
    {
  ...
  private void getConnectionConfig() {
    String result = getConfData("connectionconfig");
    if (result!=null)
    {
  ...
  private void getOIDCConfig() {
    String result = getConfData("config");
    if (result!=null)
    {
      _callbackContext.sendPluginResult(new PluginResult(PluginResult.Status.OK, result));
    }
